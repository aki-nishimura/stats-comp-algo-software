...
If you are on Linux, most likely you don't need my suggestions on what to do :-)

Besides the bare minimum Git set-up, you should set up a text editor be used by Git for commit messages and such.
I personally use Sublime Text as the Git editor on macOS and, for those on Windows, Notepad++ is one popular option. 
It is also a good idea to set up a visual diff tool, though I won't insist on this one partly because Git GUI as discussed below also provides this capability.
If you want to give visual diff a try, I like [Meld](https://meldmerge.org/) but it is currently unavailable for latest macOS versions, unfortunately;
so I have been using [KDiff3](https://apps.kde.org/kdiff3/) instead.
You should use whatever editor and visual diff you like but, to illustrate how to set them up, the following commands tell Git to use Sublime and KDiff3:
...
```{bash, eval=FALSE}
git clone https://github.com/aki-nishimura/stats-comp-algo-software-2025
```
...
Next, make a fresh clone of the repo; you can remove the old one via `rm -rf stats-comp-algo-software-2025`.
...
`rsync -P "your_username@jhpce03.jhsph.edu:/users/your_username/stats-comp-algo-software-2025/homework/git_experiment_copy/git_repo_size_history.txt" git_experiment` (assuming that you are running the command from the `homework` folder).
